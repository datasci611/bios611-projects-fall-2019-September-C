...
### The distribution of client't age and gender at time of entry. 
```{r }
ggplot(data = dat, mapping = aes(x = `Client Age at Entry`)) + 
...
ggsave('/Users/September/Documents/GitHub/bios611-projects-fall-2019-September-C/project_3/results/gender_age.png', height=4, width=7)
```  

#### Observations 

* First of all, we observe that the majority of the clients are male. 
* Most clients at the time of entry were around 40-60 years old. 
* The age range that has most female clients is between 45-50 years old. 
...
### Explore the pattern of the length of stay vs. client's income and at entry (grouped by gender). 

```{r warning=FALSE}
ggplot(dat %>% filter(`Monthly Amount (Entry)` < 30000),
       mapping = aes(x = `Monthly Amount (Entry)`, y = `Days Stayed`, colour = `Client Gender`)) + 
  geom_point() + 
  labs(title = "Scatterplot for staying days and income at entry")

ggsave('/Users/September/Documents/GitHub/bios611-projects-fall-2019-September-C/project_3/results/stay_income.png', height=4, width=8)
```    
...
* Female clients cluster toward the lower incomes.  

### Explore the pattern of the length of stay vs. client's number of noncash services at time of entry (grouped by gender) 
```{r warning=FALSE}
ggplot(dat,mapping = aes(x = `Number of Noncash at Entry`, y = `Days Stayed`, colour = `Client Gender`)) + 
  geom_point() + 
